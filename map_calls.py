...
class MapCalls (analysis.AnalysisModel) :
    def __init__ (self) :
        super(MapCalls, self).__init__()
        self.llil_handler_print = False

    def map_calls_bv (self, bv) :
        calls = {}
        for function in bv.functions :
            graph = varslice.graph_function_low_level_il(function)
            calls[function.start] = self.map_calls_llil_graph(graph)
        return calls

    def map_calls_llil_graph (self, graph) :
        calls = []
        mapped_calls = self.map_instructions(graph)
        for address in mapped_calls :
            call = mapped_calls[address]
            if call != None :
                calls.append(call)
        return calls
